      ...
      // Detect color variable changes by comparing current hash to last-published hash
        ...
        if (!globalChanged && cssAffectedPageIds.length > 0) {
          try {
            ...
            const relayerCount = await batchPublishPageLayers(cssAffectedPageIds);
            if (relayerCount > 0) {
              publishedPageIds.push(...cssAffectedPageIds);
              console.log(`[Cache] CSS catch-up: republished ${relayerCount} page layer(s)`);
            }
          } catch {
