 ...
 *
 * Overlay test headers win when fail-closed is on. Otherwise prefer
 * x-forwarded-host, except when that value is the pooled site / apex
...
export function requestHostname(
  ...
  const suffix = tenantDomainSuffixFromEnv() || env.TENANT_DOMAIN_SUFFIX?.trim();
