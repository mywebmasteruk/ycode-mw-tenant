...
export async function publishLocalisation(): Promise<PublishLocalisationResult> {
  ...
  const { data: existingPublishedLocalesRaw } = await publishedLocalesQuery;

  // Translations have no tenant_id; scope via this tenant's published locale ids.
  const publishedLocaleIds = (existingPublishedLocalesRaw || []).map((l) => l.id);
  let existingPublishedTranslationsRaw: Translation[] | null = [];
  if (publishedLocaleIds.length > 0) {
    const { data: publishedTranslations } = await client
      .from('translations')
      .select('*')
      .eq('is_published', true)
      .in('locale_id', publishedLocaleIds);
    existingPublishedTranslationsRaw = publishedTranslations;
  }
  ...
  // Step 4: Fetch draft translations for this tenant's locales only
  const tenantLocaleIds = (allDraftLocales || []).map((l: Locale) => l.id);
  let allDraftTranslations: Translation[] | null = [];
  if (tenantLocaleIds.length > 0) {
    const { data: draftTranslations, error: translationsError } = await client
      .from('translations')
      .select('*')
      .eq('is_published', false)
      .in('locale_id', tenantLocaleIds);

    if (translationsError) {
      throw new Error(`Failed to fetch draft translations: ${translationsError.message}`);
    }
    allDraftTranslations = draftTranslations;
  }
