...
async function fetchPageByPathInternal(
  ...
  try {
    ...
    if (targetPath === '' && detectedLocale) {
      ...
      const homepageData = await fetchHomepage(isPublished, paginationContext, components, tenantId ?? undefined, translations);
      if (homepageData) {
...
export async function fetchErrorPage(
  ...
  try {
    ...
    // `not-found.tsx` calls this inside `unstable_cache`, which detaches the
    // AsyncLocalStorage tenant context, so accept an explicit override and fall
    // back to the resolved tenant when called from a live request context.
    const tenantId = tenantIdOverride ?? (await resolveEffectiveTenantId());
